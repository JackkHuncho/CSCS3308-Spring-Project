...
app.post('/register', async (req, res) => {
  ...
  try {
    ...
    if (user) {
      return res.status(409).render('pages/register', { message: 'Username already exists' }).json({ message: 'Username already exists' })
    }
    ...
    return res.status(409).render('pages/register', { message: 'Registration failed due to an internal error' }).json({ message: 'Username already exists' })
  }
});
